...
![Список предприятий](docs/images/enterprises.png)
...
ТС относится к предприятию и имеет бренду (бренд описывается типом, КПП, объёмом и
мощностью двигателя, числом мест). За автомобилем закреплены водители, один из
которых может быть активным.

![Карточка автомобиля](docs/images/vehicle-info.png)
...
цветом. Выдача точек по REST доступна в формтае списка или GeoJSON.

![Несколько треков на карте](docs/images/tracks-map.png)

### Наблюдение за автомобилем

За положением каждого ТС можно наблюдать через UI. Для демонстрации координаты **симулируются**. 
Для продакшен использования достаточно добавить считывание местоположения с CAN-шины.

![Онлайн-слежение](docs/images/online-map.png)
...
![Отчёт по годам производства](docs/images/report.png)
...
![Ответ REST API](docs/images/rest-response.png)
...
будет достпно на `http://localhost:8080`, войти можно как `АнисимовВС` (пароль - в
